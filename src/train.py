...
def run_training(hp: dict):
    ...
    if checkpoint_path.exists():
        ...
        model.train(resume=True, device=train_cfg.get("device", 0))
    ...
    model_weights = hp.get("model", "yolov8m.pt")
    if not Path(model_weights).exists() and not _internet_available():
        raise RuntimeError(
            f"No internet and no local {model_weights} found. "
            f"Either connect to the internet or place {model_weights} in the project root."
        )
