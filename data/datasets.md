...
| `person` (2) | person (16) | person (XML, currently discarded) | person (2) | person (0) |
...
| **Raw source** | `data/VOC2028/` — Pascal VOC XML, 7,581 images total |
| **Raw classes** | `hat`, `person` (VOC XML `<name>` strings) |
| **Location** | `data/shwd/` (converted output) |
| **Config** | `data/shwd/shwd.yaml` |
| **Integration status** | ⚠️ Partially integrated — `person` currently discarded in conversion |

### VOC2028 Raw Structure
```
data/VOC2028/
...
- Keeps only `hat` / `helmet` XML objects → writes as YOLO class id `4`
- Discards all `person` annotations
- Only processes `test.txt` split (1,517 images → 663 label files)

### Canonical Remapping (SHWD → benchmark)
```
hat/helmet → 0  (hard_hat)
person     → 2  (person)   ← currently discarded, needs fix
```

> **Action required:** Update `convert_shwd.py` to also extract `person` boxes and write them as class `2`.
...
| **Location** | `data/CHV_dataset/` |
| **Split** | Train: 1,064 / Valid: 133 / Test: 133 |
| **Split lists** | `data/CHV_dataset/data split/train.txt` etc. |
...
| SHWD | Target (eval) | ✅ | ❌ | ⚠️ pending fix | 1,517 |
